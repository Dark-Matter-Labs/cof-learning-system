...
export function NodeDetailPanel({ node, edges, allNodes, onClose }: NodeDetailPanelProps) {
  ...
  return (
    <div className="absolute right-0 top-0 bottom-0 w-72 bg-gray-950 border-l border-gray-800 p-4 overflow-y-auto">
      ...
      {extraction?.summary && (
        <p className="text-xs text-gray-400 mb-3 leading-relaxed">{extraction.summary}</p>
      )}

      {/* Structured claim */}
      {extraction?.structured_claim && (
        <div className="bg-gray-900 rounded-md p-2.5 mb-3 border border-gray-800">
          <div className="text-[10px] text-gray-600 uppercase mb-1">Structured Claim</div>
      ...
      {extraction?.open_questions && extraction.open_questions.length > 0 && (
        <div className="mb-3">
          <div className="text-[10px] text-gray-600 uppercase mb-1">Open Questions</div>
          <ul className="space-y-1">
            {extraction.open_questions.map((q, i) => (
              <li key={i} className="text-xs text-gray-500 leading-relaxed">• {q}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {connections.length > 0 && (
        <div className="border-t border-gray-800 pt-3">
          <div className="text-[10px] text-gray-600 uppercase mb-2">Connections ({connections.length})</div>
          {connections.map(edge => {
            const isSource = edge.source_id === node.id;
            const otherNodeId = isSource ? edge.target_id : edge.source_id;
            const otherNode = nodeMap.get(otherNodeId);
            return (
              <div key={edge.id} className="text-xs text-gray-400 mb-1.5">
                <span className="text-gray-600">{isSource ? '→' : '←'}</span>{' '}
      ...
      <div className="border-t border-gray-800 pt-3 mt-3">
        <div className="text-[10px] text-gray-600 uppercase">Created</div>
